...
色々記載していますが、結論を知りたい方は[結局どうすればいいか](#結局どうすればいいか)をご覧ください。
...
- 町域名を置換する必要がある場合の考慮
  - 末尾2桁が`00`で終わる郵便番号で、町域が「以下に掲載がない場合」 → 町域を削除（[2-1](#2-1-該当する住所がなく下2桁が-00-の郵便番号が割り当てられている場合)）
  - 町域が「<任意の文字列>一円」を含む → 町域を削除（[2-2](#2-2-ある地域全体を表す一円が町域に含まれる場合)）
  - 町域が「次に」を含む → 町域を削除（[2-3](#2-3-の次にがくる場合)）
  - `（）`で補足されている町域 → `（）`内の記述を削除（[2-5](#2-5-その他で但し書きがされている場合)）
  - 町域が`、`で区切られている → 町域名を削除するか、`、`で分割して別レコードとして表示した上で選択させる（[2-4](#2-4-町域がで区切られる場合)）
