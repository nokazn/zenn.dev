...
| [〒3994511](https://www.post.japanpost.jp/cgi-zip/zipcode.php?pref=20&city=1203850&id=76100)         | 長野県   | 上伊那郡南箕輪村 | 南箕輪村一円 |
...
| [〒7613110](https://www.post.japanpost.jp/cgi-zip/zipcode.php?pref=37&city=1373640&id=138518&merge=) | 香川県   | 香川郡直島町     | 直島町一円   |
...
どれも見た目にもインパクトのある住所です。

3つ目の[〒921-0846](https://www.post.japanpost.jp/cgi-zip/zipcode.php?zip=921-8046)は[石川県金沢市三小牛町（ヘ）](https://www.post.japanpost.jp/cgi-zip/zipcode.php?pref=17&city=1172010&id=66643&merge=)にも紐づいています。\
...
- 複数の町域に紐づく郵便番号が入力されていた場合の考慮
  - 都道府県名（1.2）、市区町村名（1.1）まで表示した上で選択させ、セットさせる必要がある
- 町域名を置換する必要がある場合の考慮
  - 末尾2桁が`00`で終わる郵便番号で、町域が「以下に掲載がない場合」 → 町域を削除
  - 町域が「次に」を含む → 町域を削除
  - 町域が「<任意の文字列>一円」を含む → 町域を削除
  - 町域が`、`で区切られている → 町域名を削除するか、`、`で分割して選択させる
  - `（）`で補足されている町域 → `（）`内の記述を削除
